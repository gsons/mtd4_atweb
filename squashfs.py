    ...
    if sys.argv[1] == 'output':
        ...
        if len(squashfs_files_list) > 0:
            ...
            if len(squashfs_files_list) > 0:
                ...
                if input_image_size > image_file_size:
                    print(f'导入文件大小({input_image_size/1024})超过镜像文件({image_file_size/1024})，生成镜像失败！')
                    exit(1)

                if input_image_size > original_image_size:
                    print(f'导入文件大小超过原始文件大小，可能导致文件结构破坏，建议新镜像文件≤原镜像文件')
                shutil.copy2(image_file, image_file + ".new.bin")
                with open(image_file + ".new.bin", 'r+b') as file_a:
